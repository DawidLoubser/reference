...
import za.co.solmstraining.j2me.xml.*;
...
public class MobileOrder extends MIDlet
{
            ...
            setCommandListener(new CommandListener()
            {
                public void commandAction(Command cmd, Displayable d)
                {
                    ...
                    if (cmd == okCommand)
                    {
                        ...
                        {
                            Order order = new za.co.solmstraining.j2me.xml.OrderPersistance().openOrder();
                            //System.out.println(order);
                            display.setCurrent(new ShoppingBasketPresentation(order.getShoppingBasket()));
                        }
                        else
                            display.setCurrent
    ...
    {
        public ShoppingBasketPresentation(ShoppingBasket shoppingBasket)
        {
            ...
            if (basket == null)
                basket = new ShoppingBasket(products);
    ...
    {
        public CheckoutPresentation(ShoppingBasket theBasket)
        {
            super("Checking out.");
            System.out.println("checking out");
            this.basket = theBasket;
            ...
            setCommandListener(new CommandListener()
            {
                public void commandAction(Command cmd, Displayable d)
                {
                    ...
                    if (cmd == checkoutCmd)
                    {
                        Order order = new Order(nameField.getString(),
                        creditCardNumberField.getString(),
                        creditCardTypeSelector.getString(creditCardTypeSelector.getSelectedIndex()),
                        deliveryDateField.getDate().toString(), basket);
                        try
                        {
                            generator.getOrderFromNode(generator.getXMLFromOrder(order));
                        }catch(Exception exc)
                        {exc.printStackTrace();}
                        new za.co.solmstraining.j2me.xml.OrderPersistance().saveOrder(order);
                        System.out.println(order);
                        display.setCurrent
                        (new Alert("MobileOrder",
                        "Thank you for the order.", logo,
                        AlertType.INFO), new ShoppingBasketPresentation(basket));
                    }
                }
            });
        }
    ...
    private OrderXMLGenerator generator = new OrderXMLGenerator();
