...
public class ShoppingCartPanel extends JPanel
{
  ...
  private void connectToBean()
  {
    ...
    try
    {
      System.out.println("reading handle.dat");
      in = new ObjectInputStream
        (new FileInputStream("handle.dat"));
      System.out.println("retrieved handle.");
      javax.ejb.Handle handle = (javax.ejb.Handle)in.readObject();
//      cart = (ShoppingCart)handle.getEJBObject();
      cart = (ShoppingCart)javax.rmi.PortableRemoteObject.narrow
                 (handle.getEJBObject(), ShoppingCart.class);
      cart.getContent();
      System.out.println("read in handle");
    }
    catch (Exception e)
    {
      e.printStackTrace();
      System.out.println("Could not connect via handle.");
      createNewBeanInstance();
      System.out.println("Created new bean instance.");
    }
    finally
    {
      try
  ...
  private void createNewBeanInstance()
  {
    ...
    try
    {
      ...
      System.out.println("created new handle");
    }
    catch (javax.naming.NamingException e1)
    {
      System.out.println
        ("Could not find bean " + beanName +".");
      System.exit(0);
    }
    catch (Exception e2)
    {
      System.out.println
        ("Could not connect to bean " + beanName +".");
      System.exit(0);
    }
  }
  ...
  public void finalize()
  {
    try
    {
      System.out.println("finalizing: saving handle.");
